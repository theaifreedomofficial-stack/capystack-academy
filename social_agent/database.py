...
def delete_campaign(campaign_id: str):
    with get_db() as db:
        db.execute("DELETE FROM agent_logs WHERE job_id IN (SELECT id FROM content_jobs WHERE campaign_id=?)", (campaign_id,))
        db.execute("DELETE FROM content_jobs WHERE campaign_id=?", (campaign_id,))
...
def get_stats() -> dict:
    with get_db() as db:
        ...
        pending_approval = db.execute(
            "SELECT COUNT(*) FROM content_jobs WHERE approval_status='pending' AND job_status='completed'"
        ).fetchone()[0]
        currently_running = db.execute(
            "SELECT COUNT(*) FROM content_jobs WHERE job_status='running'"
        ).fetchone()[0]
        total_approved = db.execute(
            "SELECT COUNT(*) FROM content_jobs WHERE approval_status='approved'"
        ).fetchone()[0]
        total_rejected = db.execute(
            "SELECT COUNT(*) FROM content_jobs WHERE approval_status='rejected'"
        ).fetchone()[0]
        return {
